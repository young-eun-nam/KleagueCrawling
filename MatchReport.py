...
def getData(league_num, league_str, data):
    if league_num == "1":
    ...
    for i in tqdm(range(game_number)):  # 한번에 크롤링할 페이지 수를 설정해줄 수 있음
        try:
            row_data = []
            if league_num == "1":
                report_html = urlopen(REPORTURL + IPTYEAR + IPTGAMEID + str(i+1) + IPTSEQ + league_num).read()  # 데이터 보고서 페이지 url
            elif league_num == "2":
                report_html = urlopen(REPORTURL + IPTYEAR + IPTGAMEID + str(i+1) + IPTSEQ + league_num).read()  # 데이터 보고서 페이지 url
            else:
                pass
...
def setBasicInfo(league_num, league_str):
    ...
    result = []
    data = []
    data = getData(league_num, league_str, data)
    result.extend(data)
    return result
